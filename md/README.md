...
```
#### 热立图的流程（后面继续补充）
>1 热力图需要数据，数据从三个数据量，sql redis mongdb，数据库的数据从哪里来的？   
...
-  amd Asynchronous Module Definition 异步加载模块就不会影响后面语句的执行 define
- nodejs 是commonjs的加载 同步记载
